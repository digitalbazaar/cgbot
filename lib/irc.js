...
export async function connect(
  {server, port, channel, nick, logMessage, servpass, chanpass}) {

  channel = '#' + channel;
  let channelWithOptPass = '#' + channel;

  // check to see if there is a channel password
  if(chanpass) {
    channelWithOptPass += ' ' + chanpass;
  }
  ...
  const say = ircClient.say.bind(ircClient, channel);
  ...
  ircClient.on('join', function(channel, nick) {
    ...
    ircClient.on('message#' + channel, function(nick, message) {
      ...
      if(command.search(/^connections.*/) === 0 && args.length === 0) {
        const participantList = Object.keys(participants).map(
          channel => prettyPrintChannel(channel));
...
function prettyPrintChannel(channel) {
  let prettyPrintedChannel = 'unknown participant';
