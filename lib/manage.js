        ...
        if(stanza.attrs.from in participants) {
          ...
          if(msg && msg.event == 'SPEECH' && !msg.is_interim) {
            ...
            if(rawMessage.split(' ').length > 2) {
              ...
              ircSay(`${nick}: ${message}`);
              await xmppSay({
                info: `${nick}: ${message}`, meeting, xmppClient
              });
            } else {
              console.log('Skipping short transcription: ' + rawMessage);
            }
          }
          ...
          if(msg && msg.type === 'new-poll') {
            ...
            if(msg.pollCreatorNick) {
              // If poll is created from chat, the sender/participant is the
              // bot, while the actual sender nick should be set in the
              // pollCreatorNick property and in the question prefix,
              // as set in command.js.
              const prefix = `<${msg.pollCreatorNick}> `;
              if(typeof question === 'string' && question.startsWith(prefix)) {
                // Remove nick from question
                question = question.substr(prefix.length);
              }
              name = msg.pollCreatorNick;
            }
            const poll = {num: pollNum, msg, question, votes: {}};
            polls.byId[msg.pollId] = poll;
            ...
            const message2 = `POLL: ${poll.question}`;
            ircSay(message2);
            if(ircOptions.log) {
          ...
          if(bodyNode) {
            ...
            if(nick !== 'CG_Bot' && !replay) {
              ...
              ircSay(`${nick}: ${message}`);
              processCommand(
                {nick, message, participants, polls,
                botNick: ircOptions.nick,
                say: async message => {
                  ircSay(message);
                  await xmppSay({info: message, meeting, xmppClient});
                },
