    ...
    xmppClient.on('stanza', async stanza => {
      ...
      if(stanza.attrs.type === 'groupchat') {
        if(stanza.attrs.from in participants) {
          ...
          const transcriptNode = stanza.getChild('json-message');
          const bodyNode = stanza.getChild('body');

          // the message is a transcription message
          if(transcriptNode) {
            const transcript = JSON.parse(transcriptNode.text());
            if(transcript.event == 'SPEECH' && !transcript.is_interim) {
              const commenter = transcript.participant.name.replace(/ /g, '_');
              const rawMessage = transcript.transcript[0].text + '.';
